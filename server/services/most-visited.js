...
app.get('/api/most-visited', checkDbConnection, async (req, res) => {
    try {
      const eventsCollection = db.collection('visited');
  ...
  connectToDatabase().then(() => {
    app.listen(PORT, () => {
      console.log(` Most Visited Service running on port ${PORT}`);
    });
  });